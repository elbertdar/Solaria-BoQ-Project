...
import {
  scheduleForProject, scheduleCounts, matchesFilter, agendaBuckets, todayLocal,
...
const BORDER = '#E5E7EB';
const SHADE = 'rgba(245,158,11,0.10)';
...
export default function SchedulePage() {
  ...
  const { axis, lines, start, curOff } = scheduleForProject(db, currentProjectId, today);
  ...
  return (
    <>
      <div className="page-head">
        <h1>Schedule</h1>
        <p className="sub">{project.name} · planned in days after project start · order days are auto-computed from lead times</p>
      </div>

      <ProjectBar />

      {/* project start control */}
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap', margin: '0 0 18px', fontSize: 13, color: '#64748B' }}>
        ...
        <input type="date" value={start ? start.toISOString().slice(0, 10) : ''}
          onChange={(e) => updateProject(currentProjectId, { startDate: e.target.value || null })}
          style={{ width: 170 }} />
        {start
          ? <span className="pill gray">Today is day {curOff}</span>
          : <span style={{ color: TONE.risk }}>set a start date to place items on the calendar</span>}
      </div>

      <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', marginBottom: 16 }}>
        <Chip active={filter === 'order'} onClick={() => toggleFilter('order')}
          n={counts.toOrder} label="To order this week"
          extra={counts.overdueOrder > 0 ? <span style={{ color: TONE.risk, fontWeight: 600 }}>{counts.overdueOrder} overdue</span> : 'on track'} />
        <Chip active={filter === 'needed'} onClick={() => toggleFilter('needed')}
          n={counts.needed} label="Needed this week" extra="delivery due this week" />
        <Chip active={filter === 'arriving'} onClick={() => toggleFilter('arriving')}
          n={counts.arriving} label="Arriving this week"
      ...
      {visible.length === 0 ? (
        <div className="card"><div className="empty">
          {lines.length === 0 ? 'No BoQ items on this project yet.' : 'Nothing matches this filter.'}
        </div></div>
      ) : view === 'timeline' ? (
        <Timeline axis={axis} lines={visible} db={db} ActionButton={ActionButton} />
      ) : (
        <Agenda lines={visible} axis={axis} db={db} ActionButton={ActionButton} />
      )}
      ...
      {receiveFor && (
        <ReceiveModal
          title={`Mark received · ${receiveFor.line.materialName}`}
...
// ---------- Timeline ----------
function Timeline({ axis, lines, db, ActionButton }) {
  const N = axis.columns.length;
  const cols = `220px repeat(${N}, minmax(64px, 1fr))`;
    ...
    <div style={{ overflowX: 'auto', border: '1px solid ' + BORDER, borderRadius: 10, background: '#fff' }}>
      <div style={{ minWidth: 760 }}>
        <div style={{ display: 'grid', gridTemplateColumns: cols, alignItems: 'stretch', borderBottom: '1px solid ' + BORDER }}>
          <div style={{ padding: '9px 12px', fontSize: 12, fontWeight: 600, color: '#94A3B8', borderRight: '1px solid ' + BORDER }}>Material</div>
          {axis.columns.map((c) => (
            <div key={c.index} style={{
              padding: '7px 4px', textAlign: 'center', borderLeft: '1px dashed ' + BORDER,
              background: c.isThisWeek ? SHADE : 'transparent',
            }}>
              <div style={{ fontSize: 11.5, fontWeight: c.isThisWeek ? 700 : 500, color: c.isThisWeek ? '#0F172A' : '#64748B' }}>{c.label}</div>
              {c.date && <div style={{ fontSize: 10, color: '#94A3B8' }}>{fmtDate(c.date)}</div>}
            </div>
          ))}
        </div>
        ...
        {groups.map((g) => (
          <div key={g.key}>
            ...
            {g.rows.map((l) => (
              <Row key={l.boqItem.id} l={l} N={N} cols={cols} baseWeek={axis.baseWeek} ActionButton={ActionButton} />
            ))}
          </div>
        ))}
      </div>
...
function Row({ l, N, cols, baseWeek, ActionButton }) {
  const toCol = (wk) => (wk == null ? null : Math.max(0, Math.min(N - 1, wk - baseWeek)));
  const ow = toCol(l.orderWeek);
  const dw = toCol(l.neededWeek);
  ...
  const tip = [
    ...
    l.hasLead ? `Lead ${l.lead}d` : 'No lead time set',
  ...
  const markerBase = {
    gridRow: 1, alignSelf: 'center', justifySelf: 'center', zIndex: 2,
    width: 13, height: 13, border: '2px solid #fff', boxShadow: '0 0 0 1px rgba(15,23,42,0.2)',
  };

  return (
    <div style={{ display: 'grid', gridTemplateColumns: cols, alignItems: 'center', minHeight: 48, borderBottom: '1px solid ' + BORDER }} title={tip}>
      <div style={{ padding: '8px 12px', display: 'flex', flexDirection: 'column', gap: 5, borderRight: '1px solid ' + BORDER }}>
      ...
      {lo != null && (
        <span style={{ gridColumn: `${2 + lo} / ${2 + hi + 1}`, gridRow: 1, alignSelf: 'center', height: 8, borderRadius: 999, margin: '0 4px', background: color, zIndex: 1 }} />
      )}
      {ow != null && (
        <span style={{ ...markerBase, gridColumn: 2 + ow, borderRadius: '50%', background: color }} />
      )}
      {dw != null && (
        <span style={{ ...markerBase, gridColumn: 2 + dw, borderRadius: 3, transform: 'rotate(45deg)', background: color }} />
      )}
    </div>
  );
}
...
function LineTags({ l }) {
  ...
  return (
    <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap' }}>
      {tags.map(([t, fg, bg]) => (
        <span key={t} style={{ fontSize: 10, fontWeight: 700, padding: '1px 6px', borderRadius: 999, textTransform: 'uppercase', letterSpacing: '0.03em', color: fg, background: bg }}>{t}</span>
      ))}
    </div>
  );
}
...
// ---------- Agenda ----------
function Agenda({ lines, axis, db, ActionButton }) {
  const buckets = agendaBuckets(lines, axis);
  const mandorName = (id) => db.mandors.find((m) => m.id === id)?.name || 'Unassigned';
  ...
  return (
    <div style={{ display: 'flex', gap: 16, flexWrap: 'wrap', alignItems: 'center', fontSize: 12, color: '#64748B', margin: '12px 2px 0' }}>
      ...
      <span style={{ display: 'inline-flex', alignItems: 'center', gap: 6, marginLeft: 8 }}>
        <span style={{ width: 11, height: 11, borderRadius: '50%', border: '2px solid #fff', boxShadow: '0 0 0 1px rgba(15,23,42,.3)', background: '#94A3B8' }} /> order day
      </span>
      <span style={{ display: 'inline-flex', alignItems: 'center', gap: 6 }}>
        <span style={{ width: 10, height: 10, border: '2px solid #fff', boxShadow: '0 0 0 1px rgba(15,23,42,.3)', background: '#94A3B8', transform: 'rotate(45deg)' }} /> needed day
      </span>
      <span style={{ color: '#94A3B8' }}>· bar length = lead time</span>
    </div>
  );
}